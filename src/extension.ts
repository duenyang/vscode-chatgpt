...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const freeText = vscode.commands.registerCommand("ChatGPT-Robot.freeText", async () => {
    ...
    const resetThread = vscode.commands.registerCommand("ChatGPT-Robot.clearConversation", async () => {
		provider?.sendMessage({ type: 'clearConversation' }, true);
	});

    const exportConversation = vscode.commands.registerCommand("ChatGPT-Robot.exportConversation", async () => {
		provider?.sendMessage({ type: 'exportConversation' }, true);
	});

    const clearSession = vscode.commands.registerCommand("ChatGPT-Robot.clearSession", () => {
    ...
    const adhocCommand = vscode.commands.registerCommand("ChatGPT-Robot.adhoc", async () => {
    ...
    const generateCodeCommand = vscode.commands.registerCommand(`ChatGPT-Robot.generateCode`, () => {
		const editor = vscode.window.activeTextEditor;
    ...
    const registeredCommands = menuCommands.filter(command => command !== "adhoc" && command !== "generateCode").map((command) => vscode.commands.registerCommand(`ChatGPT-Robot.${command}`, () => {
