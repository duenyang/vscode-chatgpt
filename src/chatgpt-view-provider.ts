...
export default class ChatGptViewProvider implements vscode.WebviewViewProvider {
		...
		webviewView.webview.onDidReceiveMessage(async data => {
			switch (data.type) {
				...
				case 'login':
					...
					break;
				case 'openSettings':
                    vscode.commands.executeCommand('workbench.action.openSettings', "@ext:duduyoung.ChatGPT-Robot chatgpt.");

					this.logEvent("settings-opened");
					break;
				case 'openSettingsPrompt':
                    vscode.commands.executeCommand('workbench.action.openSettings', "@ext:duduyoung.ChatGPT-Robot promptPrefix");
	...
	private get systemContext() {
        ...
        return `你是帮助用户编码的ChatGPT。 
			你很聪明，乐于助人，是一个开发专家，总是给出正确的答案，只做被要求的事情。你总是如实回答，不瞎编。
			(在响应以下提示时，请确保使用Github主题的Markdown正确地设置您的响应。
			对标题、列表、彩色文本、代码块、突出显示等内容使用 markdown 语法。确保不要在实际响应中提及 markdown 或样式。)`;
	}
	...
	public async sendApiRequest(prompt: string, options: { command: string, code?: string, previousAnswer?: string, language?: string; }) {
		...
		if (this.webView == null) {
            vscode.commands.executeCommand('ChatGPT-Robot.view.focus');
		} else {
		...
		try {
			...
			if (this.subscribeToResponse) {
				vscode.window.showInformationMessage("ChatGPT responded to your question.", "Open conversation").then(async () => {
                    await vscode.commands.executeCommand('ChatGPT-Robot.view.focus');
				});
			}
		} catch (error: any) {
			let message;
			...
			if (error?.response?.status || error?.response?.statusText) {
				...
				vscode.window.showErrorMessage("An error occured. If this is due to max_token you could try `ChatGPT: Clear Conversation` command and retry sending your prompt.", "Clear conversation and retry").then(async choice => {
					if (choice === "Clear conversation and retry") {
                        await vscode.commands.executeCommand("ChatGPT-Robot.clearConversation");
						await delay(250);
